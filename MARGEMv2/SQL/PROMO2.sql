-- SQL PROMO
-- 30.06.2023
-- SANDRO JAKOSKA

WITH 
PEDID_DATE AS (
    SELECT ID_PEDIDO 
    FROM PEDID 
    WHERE PEDDTBAIXA >=DATEADD(-90 DAY TO CURRENT_DATE)
),   

PROMO AS (
    SELECT ID_PROMO, DESCRICAO 
    FROM PROMO
),

PED_CUPOM AS (
    SELECT P.ID_PEDIDO, CAST (PEDOBSER AS VARCHAR (700)) CUPOM_OBS
    FROM PEDID P
    INNER JOIN PEDID_DATE PDT ON P.ID_PEDIDO = PDT.ID_PEDIDO
),  

PED_PROMO_PLUGIN AS (
    SELECT ID_PEDIDPROMOCAO AS ID_PEDIDO_PROMO, DESCRICAO AS DESCRICAO_PROMO,UPPER(TRIM(NRVOUCHERPROMO)) CUPOM_PLUGIN, CUPOM_OBS
    FROM PEDIDPROMO P1
    INNER JOIN PEDID_DATE PDT ON P1.ID_PEDIDPROMOCAO = PDT.ID_PEDIDO
    LEFT JOIN PROMO P ON P.ID_PROMO = P1.ID_PROMO
    LEFT JOIN PED_CUPOM PC ON P1.ID_PEDIDPROMOCAO=PC.ID_PEDIDO
),

PED_PROMO_CONECTA AS (
    SELECT P2.ID_PEDIDO AS ID_PEDIDO_PROMO, PIPPROMOCAO AS DESCRICAO_PROMO,UPPER(TRIM(PIPVOUCHER)) AS CUPOM_PLUGIN, CUPOM_OBS
    FROM PDINFOPROMO P2
    INNER JOIN PEDID_DATE PDT ON P2.ID_PEDIDO = PDT.ID_PEDIDO
    LEFT JOIN PED_CUPOM PC ON P2.ID_PEDIDO=PC.ID_PEDIDO
    WHERE PIPPAR = 2
),

PED_PROMO_PAP AS (
    SELECT P3.ID_PEDIDO ID_PEDIDO_PROMO, 'PROMO EM DOBRO PAP' DESCRICAO_PROMO, NULL AS CUPOM_PLUGIN, CUPOM_OBS
    FROM PDPRD P3
    INNER JOIN PEDID_DATE PDT ON P3.ID_PEDIDO = PDT.ID_PEDIDO
    LEFT JOIN PED_CUPOM PC ON P3.ID_PEDIDO=PC.ID_PEDIDO
    WHERE P3.PROCODIGO = 'PAP'
    GROUP BY 1, 2, 3 , 4
    HAVING SUM(PDPUNITLIQUIDO * PDPQTDADE) <= 1
),

PED_MEU_1_VLX AS (
    SELECT P4.ID_PEDIDO ID_PEDIDO_PROMO, 'MEU 1 VARILUX' DESCRICAO_PROMO, NULL AS CUPOM_PLUGIN, CUPOM_OBS
    FROM PDPRD P4
    INNER JOIN PEDID_DATE PDT ON P4.ID_PEDIDO = PDT.ID_PEDIDO
    LEFT JOIN PED_CUPOM PC ON P4.ID_PEDIDO=PC.ID_PEDIDO
    WHERE P4.PROCODIGO = 'PAP'
    GROUP BY 1, 2, 3 , 4
    HAVING SUM(PDPUNITLIQUIDO * PDPQTDADE) > 1
)


SELECT DISTINCT ID_PEDIDO_PROMO AS ID_PEDIDO, DESCRICAO_PROMO,  CUPOM_PLUGIN, CUPOM_OBS
FROM PED_PROMO_PLUGIN  
UNION
SELECT DISTINCT ID_PEDIDO_PROMO AS ID_PEDIDO, DESCRICAO_PROMO,  CUPOM_PLUGIN, CUPOM_OBS
FROM PED_PROMO_CONECTA
UNION
SELECT DISTINCT ID_PEDIDO_PROMO AS ID_PEDIDO, DESCRICAO_PROMO,  CUPOM_PLUGIN, CUPOM_OBS
FROM PED_PROMO_PAP
UNION
SELECT DISTINCT ID_PEDIDO_PROMO AS ID_PEDIDO, DESCRICAO_PROMO,  CUPOM_PLUGIN, CUPOM_OBS
FROM PED_MEU_1_VLX

